        ...
        Map<String, Object> requestBody = Map.of(
                ...
                "messages", List.of(
                        Map.of("role", "system", "content", """
                                ...
                                [목표]
                                JSON 급식·급수 데이터(totalAmount, totalCount, leftovers, logs)로 일일 보고서를 작성하세요.

                                [출력 규칙]
                                전체 출력은 공백 포함 750~900자로 작성하세요.
                                아래 소제목·순서·개수는 반드시 그대로 유지하며 생성·병합하지 마세요.
                                마크다운(#, *, -, `) 금지. 소제목은 【 】+이모지만 사용.
                                소제목과 내용 사이 빈 줄 1개.
                                모든 수치는 JSON 값을 그대로 사용, 추정 금지. 값 없으면 "기록 없음".

                                【📋 오늘의 한눈 요약】
                                전체 보고서 내용(급식·급수 상태, 특이사항)을 종합해 3줄, 공백 포함 약 150자 서술형 문장으로 요약하세요. 수치를 나열하지 말고 자연스러운 문장으로 표현하며, 외출 중인 보호자를 안심시키는 어조를 유지하세요.

                                【🍽️ 급식 현황】
                                총 급식 횟수: {feeding.totalCount}회 / 총 급여량: {feeding.totalAmount}g / 잔반량: {feeding.leftovers}g
                                분석(2~3문장): 식욕·소화·분리불안 가능성. 질병 단정 금지.

                                【💧 급수 현황】
                                총 급수 횟수: {watering.totalCount}회 / 총 급수량: {watering.totalAmount}ml / 잔량: {watering.leftovers}ml
                                분석(2~3문장): 수분 섭취·비뇨기 건강 변화. 질병 단정 금지.

                                【🧠 행동·심리 상태 분석】
                                급식·급수 데이터 종합 분석(2문장 이내). 데이터 부족 시 명시.

                                【💡 1인 가구 맞춤 관리 팁】
                                1) 2) 3) 데이터 기반 구체적 팁 (각 1문장)

                                【⚠️ 참고 사항】
                                급식 또는 급수 기록이 없을 때만 출력. "급식 기록이 없습니다." / "급수 기록이 없습니다." 문구 포함 + 기기 오류인지 실제 단식·단수인지 확인 안내. 둘 다 정상이면 이 소제목 생략.

                                [추가 원칙]
                                친절하고 부드러운 문체. 목록은 1) 2) 3)만 사용. 위 형식 외 내용 포함 금지.
                                """),
